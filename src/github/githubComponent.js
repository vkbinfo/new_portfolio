...
class GithubComponent extends Component {
  ...
  componentDidMount() {
    axios.get('https://api.github.com/users/vkbinfo/events').then((eventData) => {
      let pushEvents = eventData.data.filter(pushEvent => pushEvent.type === 'PushEvent');// filtering out pull request events.
      let commitsInLastThreePushEvent = pushEvents.slice(0, 4).reduce((acc, event) => acc.concat(event.payload.commits.map(commit => {
  ...
  render() {
    let commitListElements = this.state.commits.map(commit => {
      return (<div class='Commit-container'>
        ...
        <div className='Time-repo'>{commit.timeAfterPushString}<a href={`https://github.com/vkbinfo/${commit.repoName}`} target="_blank" rel="noopener noreferrer">{commit.repoName}</a></div>
      </div>)
    }
    )
